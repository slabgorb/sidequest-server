...
``process_room_entry`` has four return points (room_movement.py:91, 101,
105, 121) with zero watcher events. Per CLAUDE.md "OTEL Observability
Principle," every backend decision path must emit a span so the GM
panel (Sebastien-mode lie detector) can verify the hook engaged.
...
* ``room.entry_skipped`` — fires when process_room_entry returns
  ...
  - ``reason``: one of ``"not_chassis_room"``, ``"no_bond_for_actor"``,
    ``"no_magic_state"``, ``"chassis_not_found"``
...
@pytest.mark.integration
def test_skipped_span_emitted_when_magic_state_missing(otel_capture) -> None:
    """Snapshot with chassis_registry but magic_state=None must emit
    room.entry_skipped reason=no_magic_state and not raise."""
    from sidequest.game.room_movement import process_room_entry
    from sidequest.telemetry.spans.rig import SPAN_ROOM_ENTRY_SKIPPED

    snap = _bootstrap_coyote_star_snapshot()
    # Force the post-init drift: a corrupted save where magic_state was
    # cleared but chassis_registry persisted.
    snap.magic_state = None
