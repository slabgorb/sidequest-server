...
# ---------------------------------------------------------------------------
# Sünden engine plan item 4a — hireling_id + is_dead
# ---------------------------------------------------------------------------


def _make_minimal_character(
    *,
    name: str = "Anon",
    hireling_id: str | None = None,
) -> Character:
    """Minimal Character builder for plan-item-4a coverage.

    Mirrors ``make_test_character`` but exposes the two new fields
    directly so the round-trip / default tests are tight.
    """
    return Character(
        core=CreatureCore(
            name=name,
...
def test_character_hireling_id_defaults_none():
    ch = _make_minimal_character(name="Anon")
    assert ch.hireling_id is None


def test_character_hireling_id_round_trips():
    ch = _make_minimal_character(name="Volga", hireling_id="vol_1")
