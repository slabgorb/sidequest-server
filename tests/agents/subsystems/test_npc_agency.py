...
async def test_npc_agency_handles_npc_with_null_role_and_location():
    ...
    out = await run_npc_agency(dispatch, npc_registry=registry)
    assert len(out.directives) == 1
    payload = out.directives[0].payload
    # No double spaces anywhere.
    assert "  " not in payload
    # No dangling semicolon before period.
    assert "; ." not in payload
...
@pytest.mark.asyncio
async def test_npc_agency_case_insensitive_lookup(minimal_npc_registry):
    """Registry stores 'Harlan'; decomposer may emit 'harlan' — both should resolve."""
    dispatch = SubsystemDispatch(
        subsystem="npc_agency",
    ...
    out = await run_npc_agency(dispatch, npc_registry=minimal_npc_registry)
    assert out.directives != []
    assert out.data["npc_name"] == "Harlan"  # returns the canonical registry casing
